...
export default function RegisterPage() {
  ...
  const validateClient = (): FieldErrors => {
    const e: FieldErrors = {};
    if (!displayName.trim()) e.displayName = "表示名（ユーザー名）を入力してください";
    ...
    if (displayName.trim() && displayName.trim().length < 3) {
      e.displayName = "表示名（ユーザー名）は3文字以上で入力してください";
    }
    ...
    <div className="mx-auto max-w-md px-4 py-10">
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm p-6">
        <div className="text-center">
          <div className="text-2xl font-extrabold text-slate-900">ユーザー登録</div>
        ...
        <div className="mt-6 space-y-4">
          <div>
            <div className="text-sm font-semibold text-slate-700 mb-1">表示名（ユーザー名） *</div>
            <input
              className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm"
              value={displayName}
              onChange={(e) => setDisplayName(e.target.value)}
              placeholder="表示名（ユーザー名）を入力"
            />
            {errors.displayName && (
